...
import { CardRiverStateType, CardConfig } from "./CardRiverActionsReducers";
...
const CardRiver = ({ index, state }: { index: number; state: CardRiverStateType }) => {
	return (
		<Accordion>
			<AccordionSummary>CardRiver</AccordionSummary>
			<AccordionDetails>
				<Grid container direction="column" spacing={2} justify="center" alignItems="stretch">
					{state.riverMakeUps[index] &&
						state.riverMakeUps[index].map((card) => (
							<Grid item>
...
const mapStateToProps = (state: any) => {
	return { state: state.cardRiverState };
};

export const CardRiverContainer = connect(mapStateToProps)(CardRiver);
